...
```
&emsp;&emsp;有一个未解决的问题，添加的`config`文件必须在`/c/users/用户名/.ssh/`文件夹下才生效，而且，每次开`git-bash`都需要`ssh-agent bash`才能连接上git。
...
在下面空白行添加
`用户名   ALL=(ALL) ALL`
然后`:x`保存，再使用`chmod u-w /etc/sudoers`去掉文件的写权限。使用这种方法比较麻烦，需要修改文件的读写权限。
&emsp;&emsp;第二种方法，在`root`用户下使用`echo '用户名 ALL=(ALL) ALL' > /etc/sudoers.d/用户名`，输完命令后，将文件`用户名`的权限改为`0440`，使用命令：`chmod 0440 /etc/sudoers.d/用户名`。
&emsp;&emsp;第三种方法将用户添加到有`sudo`权限的组，之前看到的`/etc/sudoers`文件里有两段内容：
```
%admin  ALL=(ALL)  ALL
%sudo   ALL=(ALL)  ALL
```
&emsp;&emsp;这两段上面的注释内容表示，只要在`admin`和`sudo`组的用户，就有`root`权限和可以使用`sudo`命令，那么只要将我们的用户添加到这组里就可以。
&emsp;&emsp;修改用户所在的组使用命令`usermod`。
参数：
```
...
```
&emsp;&emsp;使用`usermod -aG adm,sudo 用户名`来给新建用户添加`root和sudo`权限。
